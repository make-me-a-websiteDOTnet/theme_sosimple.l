<?php
  ...
  function _sosimple_load($files) {
    $tp = drupal_get_path('theme', 'sosimple');
    $file = '';
  ...
  _sosimple_load($files);




  function sosimple_js_alter(&$javascript) {
    $jquery_path = drupal_get_path('theme','sosimple') . '/js/vendor/jquery.js';
 ...
 */
function sosimple_html_head_alter(&$head_elements) {
 ...
 */
function sosimple_breadcrumb($variables) {
...
function sosimple_field($variables) {
  $output = '';
 ...
 */
function sosimple_field__taxonomy_term_reference($variables) {
  $output = '';
 ...
 */
function sosimple_links__topbar_main_menu($variables) {
  // We need to fetch the links ourselves because we need the entire tree.
  $links = menu_tree_output(menu_tree_all_data(variable_get('menu_main_links_source', 'main-menu')));
  $output = _sosimple_links($links);
 ...
 * Formats links for Top Bar http://foundation.zurb.com/docs/components/top-bar.html
 */
function sosimple_links__topbar_secondary_menu($variables) {
  // We need to fetch the links ourselves because we need the entire tree.
 ...
 *
 * @see sosimple_links__system_main_menu()
 * @see sosimple_links__system_secondary_menu()
 */
function _sosimple_links($links) {
  $output = '';

  foreach (element_children($links) as $key) {
    $output .= _sosimple_render_link($links[$key]);
  }
 ...
 * @see _sosimple_links()
 */
function _sosimple_render_link($link) {
  $output = '';
  ...
  if (!empty($link['#href'])) {
    ...
    if (!isset($rendered_link)) {
      $rendered_link = theme('sosimple_menu_link', array('link' => $link));
    }
    ...
    if (!empty($link['#below'])) {
      ...
      foreach ($link['#below'] as $sub_link) {
        if (!empty($sub_link['#href'])) {
          $sub_menu .= _sosimple_render_link($sub_link);
        }
      }
 ...
 */
function theme_sosimple_menu_link($variables) {
  $link = $variables['link'];
 ...
 */
function sosimple_preprocess_block(&$variables) {
 ...
 */
function sosimple_preprocess_field(&$variables) {
 ...
 */
function sosimple_preprocess_html(&$variables) {
  global $language;
  ...
  $font =  theme_get_setting('font_family');
    if (theme_get_setting('font_family')) {
      drupal_add_css('http://fonts.googleapis.com/css?family=' . $font , array('type' => 'external', 'group' => CSS_THEME));
    }

  $font_array = explode(':',str_replace('+',' ',theme_get_setting('font_family')));
  $font_name = '"'.$font_array[0].'"';

  $font_family = array(
    '#type' => 'markup',
    '#markup' => "<style type='text/css'>body {font-family:".$font_name.", sans-serif !important;}</style> ",
    '#weight' => 11,
  );

  $font =  theme_get_setting('headings_font_family');
  if (theme_get_setting('headings_font_family')) {
    drupal_add_css('http://fonts.googleapis.com/css?family=' . $font , array('type' => 'external', 'group' => CSS_THEME));
  }
  $font_array = explode(':',str_replace('+',' ',theme_get_setting('headings_font_family')));
  $font_name = '"'.$font_array[0].'"';

  $headings_font_family = array(
    '#type' => 'markup',
    '#markup' => "<style type='text/css'>h1, h2, h3, h4, h5, h6 {font-family:".$font_name.", sans-serif !important;}</style> ",
    '#weight' => 12,
  );
   ...
   *   Use with caution.
   */
  // drupal_add_js('document.write(\'<script src=/' . drupal_get_path('theme', 'sosimple') .'/js/vendor/\'
 ...
 */
function sosimple_preprocess_node(&$variables) {
 ...
 */
function sosimple_preprocess_page(&$variables) {
  ...
  // Top bar.
  if ($variables['top_bar'] = theme_get_setting('sosimple_top_bar_enable')) {
    $top_bar_classes = array();

    if (theme_get_setting('sosimple_top_bar_grid')) {
      $top_bar_classes[] = 'contain-to-grid';
    }

    if (theme_get_setting('sosimple_top_bar_sticky')) {
    ...
    $variables['top_bar_menu_text'] = theme_get_setting('sosimple_top_bar_menu_text');

    $top_bar_options = array();

    if (!theme_get_setting('sosimple_top_bar_custom_back_text')) {
      $top_bar_options[] = 'custom_back_text:false';
    }

    if ($back_text = theme_get_setting('sosimple_top_bar_back_text')) {
      if ($back_text !== 'Back') {
        $top_bar_options[] = "back_text:'{$back_text}'";
      }
    }

    if (!theme_get_setting('sosimple_top_bar_is_hover')) {
      $top_bar_options[] = 'is_hover:false';
    }

    if (!theme_get_setting('sosimple_top_bar_scrolltop')) {
  ...
  // Messages in modal.
  $variables['sosimple_messages_modal'] = theme_get_setting('sosimple_messages_modal');
 ...
 */
function sosimple_css_alter(&$css) {
  // Remove defaults.css file.
  unset($css[drupal_get_path('module', 'system') . '/system.menus.css']);

  // Remove Drupal core CSS.
  if (theme_get_setting('sosimple_disable_core_css')) {
 ...
 */
function sosimple_pager($variables) {
  $tags = $variables['tags'];
  ...
  if ($pager_total[$element] > 1) {
    ...
    if (theme_get_setting('sosimple_pager_center')) {
 ...
 */
function sosimple_theme() {
  $return = array();

  $return['sosimple_reveal'] = array(
    'variables' => array(
    ...
    'function' => 'theme_sosimple_reveal',
  );

  $return['sosimple_menu_link'] = array(
    'variables' => array('link' => NULL),
    'function' => 'theme_sosimple_menu_link',
  );
  return $return;
}
 ...
 *
 * @see theme_sosimple_reveal()
 */
function _sosimple_reveal($reveal = NULL) {
 ...
 *
 * @see sosimple_theme()
 * @see sosimple_preprocess_region()
 * @see _sosimple_reveal()
 */
function theme_sosimple_reveal($variables) {
  // Generate unique IDs.
  static $counter = 0;
 ...
 */
function _sosimple_add_reveals() {
  $markup = '';

  // Retrieve reveal markup from static storage.
  foreach (_sosimple_reveal() as $reveal) {
 ...
 */
function sosimple_theme_registry_alter(&$theme_registry) {
  ...
  foreach ($entity_info as $entity => $info) {
    if (isset($entity_info[$entity]['fieldable']) && $entity_info[$entity]['fieldable']) {
      ...
      if (isset($theme_registry[$entity])) {
        $theme_registry[$entity]['preprocess functions'][] = 'sosimple_entity_variables';
      }
    }
  }
  ...
  if (isset($theme_registry['file_entity'])) {
    $theme_registry['file_entity']['preprocess functions'][] = 'sosimple_entity_variables';
  }
 ...
 *
 * @see sosimple_theme_registry_alter()
 */
function sosimple_entity_variables(&$vars) {
 ...
 */
function sosimple_process_html_tag(&$vars) {
  if (theme_get_setting('sosimple_html_tags')) {
    $el = &$vars['element'];
